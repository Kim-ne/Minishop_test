                        ...
                        <div class="bg-light p-30 mb-5 col-lg-6 offset-lg-3 rounded ">
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    <strong>Username: {{ auth()->guard('buyer')->user()->fullname }}</strong>
                                </div>

                                <div class="col-md-12 mb-3">
                                    <strong>Email: </strong>
                                    <span>{{ auth()->guard('buyer')->user()->email ?? 'empty' }}</span>
                                </div>

                                <div class="col-md-12 mb-3">
                                    <strong>Phone: </strong>
                                    <span>{{ auth()->guard('buyer')->user()->phone ?? 'empty'}} </span>
                                </div>

                                <div class="col-md-12 mb-3 ">
                                    <strong>Country: </strong>
                                    <span>{{ auth()->guard('buyer')->user()->country ?? 'empty' }}</span>
                                </div>
                    ...
                    <form action="{{ route('info.update') }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="name" class="form-control"
                                value="{{ auth()->guard('buyer')->user()->fullname }}" required disabled>
                        </div>
                        <div class="form-group mb-3">
                            <label for="">Email</label>
                            <input type="email" name="email" class="form-control"
                                value="{{ auth()->guard('buyer')->user()->email }}" required disabled>
                        </div>
                        <div class="form-group mb-3">
                            <label for="">Phone</label>
                            <input type="text" name="phone" class="form-control"
                                value="{{ auth()->guard('buyer')->user()->phone }}">
                        </div>
                        <div class="form-group mb-3">
                            <label for="">Country</label>
                            <input type="text" name="country" class="form-control"
                                value="{{ auth()->guard('buyer')->user()->country }}">
                        </div>
                        <div class="col-md-12 text-center py-4">
                            <button class="btn btn-dark btn-block btn-login" type="submit">Update</button>
                        </div>
                        <div class="col-md-12 text-center mt-2">
                            <a class="btn btn-dark btn-block btn-login" href="{{ route('profile') }}">Back</a>
                        </div>
                    </form>
                </div>
            </div>
